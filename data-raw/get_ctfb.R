#' @title Get the Taxonomic Catalog of the Brazilian Fauna (CFTB)
#'
...
url0 <- "https://ipt.jbrj.gov.br/jbrj/archive.do?r=catalogo_taxonomico_da_fauna_do_brasil"
...
if (last_updated != last_download) {
  ...
  last_update <- .getUpdates(url0, backbone, zip)
  ...
  # ## Fixing duplicated infra-specific names
  # combo <- paste(data$taxon_name, data$scientificNameAuthorship)
  # rep_these <- duplicated(combo) & data$taxonRank %in% c("SUB_ESPECIE")
  # if (any(rep_these)) {
  #   nomes_spp <- unique(combo[rep_these])
  ...
  # check_these <- data$taxonRank %in% "SUB_ORDEM" &
  #                   data$taxon_name %in% c("", " ", NA)
  # if (any(check_these))
  ...
  # check_these <- data$taxonRank %in% "SUB_CLASSE" &
  #                 data$taxon_name %in% c("", " ", NA)
  # if (any(check_these))
  ...
  data$nomenclaturalStatus <- tolower(data$nomenclaturalStatus)
  
  # Editing data --------------------------------------------------
  ## adding missing accepted names
  miss_ids <- unique(
    data$acceptedNameUsageID[!data$acceptedNameUsageID %in% unique(data$id)])
  miss_ids <- unlist(strsplit(miss_ids, ";"))
  miss_ids <- unique(
    miss_ids[!miss_ids %in% unique(data$id)])
  ...
  table(data$higherClassification, useNA = "always")
  ...
  dist$accepted.id <- as.character(dist$accepted.id)
  tmp2 <- dplyr::left_join(tmp, dist, by = "accepted.id")

  stopifnot(identical(tmp1$id, tmp2$id))
  
  rep_these <- !tmp2$taxon.distribution %in% c("", " ", NA)
  ...
  rep_these <- tmp2$taxon.distribution.x %in% c("", " ", NA) & 
                !tmp2$taxon.distribution.y %in% c("", " ", NA)
  tmp1$taxon.distribution[rep_these] <- 
    tmp2$taxon.distribution.y[rep_these] 
  stopifnot(identical(tmp1$id, data$id))
  
  tmp2$taxon.distribution.y[tmp2$taxon.distribution.y %in% "BR"] <- ""
  rep_these <- !tmp2$taxon.distribution.x %in% c("", " ", NA) & 
                !tmp2$taxon.distribution.y %in% c("", " ", NA)
  tmp1$taxon.distribution[rep_these] <- 
    paste(tmp1$taxon.distribution[rep_these],
  ...
  ## How many columns and lines (in Sept 2025: 187,582)
  dimensions <- paste0(dim(data)[1], " rows and ", dim(data)[2], " columns")
  
  ## Saving
  ctfbNames <- data
  usethis::use_data(ctfbNames, compress = "xz", overwrite=TRUE)
